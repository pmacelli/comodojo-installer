...
class Route extends AbstractAction implements ActionInterface {

    public function install($package_id, $package_extra) {

        $io = $this->getIO();

        foreach ($package_extra as $route => $config) {

            $io->write("+ Installing route $route...");
            ...
            $result = $this->getPackageInstaller()->route()->add($bundle);
    ...
    public function uninstall($package_id, $package_extra) {
        ...
        foreach ($package_extra as $route => $config) {
            ...
            $io->write("- Removing route $route...");

            $this->getPackageInstaller()->rpc()->get($route)->delete();
