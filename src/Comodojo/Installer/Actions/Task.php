...
class Task extends AbstractAction implements ActionInterface {

    public function install($package_id, $package_extra) {

        $io = $this->getIO();

        foreach ($package_extra as $task => $config) {

            $io->write("+ Installing task $task...");
            ...
            $result = $this->getPackageInstaller()->tasks()->add($bundle);
    ...
    public function update($package_id, $initial_extra, $target_extra) {
        ...
        foreach ( ArrayOps::arrayFilterByKey($update, $target_extra) as $name => $config ) {
            ...
            $io->write("~ Updating task $name...");

            $this->getPackageInstaller()->tasks()->get($name)->merge($config)->persist();
    ...
    public function uninstall($package_id, $package_extra) {
        ...
        foreach ($package_extra as $task => $config) {
            ...
            $io->write("- Removing task $task...");

            $this->getPackageInstaller()->tasks()->get($task)->delete();
