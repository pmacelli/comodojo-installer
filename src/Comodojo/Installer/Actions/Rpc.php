...
class Rpc extends AbstractAction implements ActionInterface {

    public function install($package_id, $package_extra) {

        $io = $this->getIO();

        foreach ($package_extra as $rpc => $config) {

            $io->write("+ Installing rpc method $rpc...");
            ...
            $result = $this->getPackageInstaller()->rpc()->add($bundle);
    ...
    public function update($package_id, $initial_extra, $target_extra) {
        ...
        foreach ( ArrayOps::arrayFilterByKey($update, $target_extra) as $name => $config ) {
            ...
            $io->write("~ Updating rpc method $name...");

            $this->getPackageInstaller()->rpc()->get($name)->merge($config)->persist();
    ...
    public function uninstall($package_id, $package_extra) {
        ...
        foreach ($package_extra as $rpc => $config) {
            ...
            $io->write("- Removing rpc method $rpc...");

            $this->getPackageInstaller()->rpc()->get($rpc)->delete();
