...
class Setting extends AbstractAction implements ActionInterface {

    public function install($package_id, $package_extra) {

        $io = $this->getIO();

        foreach ($package_extra as $setting => $config) {

            $io->write("+ Installing setting $setting...");
            ...
            $result = $this->getPackageInstaller()->settings()->add($bundle);
    ...
    public function update($package_id, $initial_extra, $target_extra) {
        ...
        foreach ( ArrayOps::arrayFilterByKey($update, $target_extra) as $name => $config ) {
            ...
            $io->write("~ Updating setting $name...");

            $this->getPackageInstaller()->settings()->get($name)->merge($config)->persist();
    ...
    public function uninstall($package_id, $package_extra) {
        ...
        foreach ($package_extra as $setting => $config) {
            ...
            $io->write("- Removing setting $setting...");

            $this->getPackageInstaller()->tasks()->get($setting)->delete();
